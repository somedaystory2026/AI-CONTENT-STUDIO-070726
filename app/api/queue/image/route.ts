...
export async function POST(req: Request) {
  try {
    ...
    const queueResult = await enqueueStudioJob("image", "generate-image", input);

    let job: unknown = { id: `offline-${Date.now()}`, type: "IMAGE_GENERATION", status: queueResult.queued ? "WAITING" : "COMPLETED", progress: queueResult.queued ? 0 : 100, input };
    try {
    ...
    return NextResponse.json({ success: false, message: error instanceof Error ? error.message : "이미지 작업 등록 실패" }, { status: 500 });
  }
}
